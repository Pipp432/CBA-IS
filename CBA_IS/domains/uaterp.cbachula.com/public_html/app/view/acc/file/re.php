    ...
    <div class="container mt-3" ng-controller="moduleAppController" ng-init="getDetail()">
        ...
        <div class="row px-2 mt-2">
            ...
            <div class="col-4 px-0">
                ...
                <h5 style="text-align: right;"><b>เลขที่ {{detail[0].re_no}}</b></h5>
                <h6 style="text-align: right;">วันที่ {{detail[0].re_date}}</h6>
            </div>
        </div>
...
<script>
    ...
    app.controller('moduleAppController', function($scope) {
        $scope.getDetail = function() {
            $scope.detail = <?php echo $this->re; ?>;
            $scope.company = $scope.detail[0].re_no.substring(0,1);
            $scope.year = $scope.detail[0].re_date.substring(0,4);
			$scope.month = $scope.detail[0].re_date.substring(5,7);
			$scope.day = $scope.detail[0].re_date.substring(8,10);
