...
<body>
    ...
    <div class="container mt-3" ng-controller="moduleAppController">
            ...
            <div class="card-body">
                <div class="row mx-0">
                    ...
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" ng-model="Transportation" value="no" id="Transport">
                        <label class="form-check-label" for="Transport">
                        ไม่มีค่าขนส่ง (ถ้ามีค่าส่งไม่ต้องติ้ก หรือ หาค่าส่งไม่เจอติดต่อ IS)
                        </label>
                    </div>            
        ...
        <script>
            ...
            addModal('formValidate6', 'ใบสั่งขาย / Sales Order (SO)', 'ยังไม่ได้เพิ่มค่าขนส่ง');
...
<script>
    ...
    app.controller('moduleAppController', function($scope, $http, $compile) {
        ...
        $scope.Transportation = 'yes';
        
        $scope.allProducts = <?php echo $this->products; ?>;

        $scope.addSeller = function() {
            if($scope.seller_employee_id.length === 5) {
                $http.get("/public/json/employees.json").then(function(response) {
                    var found = false;
        ...
        $scope.formValidate = function() {
            ...
            $scope.CheckTransport = 0;
            
            if($scope.Transportation == 'yes') {            
                angular.forEach($scope.soItems, function(value, key) {
                    if (value.product_type == 'Install') { // check Install
                        $scope.CheckTransport = 1;
                        if (value.product_name.includes("ส่ง")) {   // check ค่าขนส่ง
                            $scope.CheckTransport = 0; 
                            break;
                        }
                    }
        ...
        $scope.postSoItems = function() {
            ...
            $.post("sales_order/post_so_items", {
                ...
                addModal('successModal', 'ใบสั่งขาย / Sales Order (SO)', 'บันทึก ' + data + ' สำเร็จ');
