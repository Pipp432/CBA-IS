...
class finModel extends model {
    ...
    public function getSoxsForCr() {
        $sql = $this->prepare("select 
                                ...
                                left join View_InvoiceStock on  View_InvoiceStock.product_no = Product.product_no
    ...
    public function addCr() {
        ...
        foreach($crItemsArray as $value) {
            ...
            while( $accumStock > 0) {
                ...
                // echo " ".($rrTable['balance']);
                $rr_no = $rrTable['file_no'];
                if($rrStock<=0) return " <h1 style = 'color:red;'>error บอก IS ด่วน หยุดออก invoice error บอก IS ด่วน หยุดออก invoice error บอก IS ด่วน หยุดออก invoice error บอก IS ด่วน หยุดออก invoice error บอก IS ด่วน หยุดออก invoice balance <= 0</h1>";
