...
class finModel extends model {
    ...
    public function postAdditionReReqDetail(){
        ...
        $sql = $this->prepare("update Reimbursement_Request set Reimbursement_Request.evidence=?,Reimbursement_Request.company=?,Reimbursement_Request.ex_no=?,Reimbursement_Request.authorize_date=?,Reimbursement_Request.authorizer_name=? where re_req_no=? ");
    ...
    public function addCr() {
        ...
        $CheckSoTime = '';
        foreach($crItemsArray as $value) {
            $CheckSoDate = substr($value['sox_datetime'],0,9);
            $CheckSoTime = substr($value['sox_datetime'],9);
            if (array_key_exists($value['so_no'], $soList)) {
                ...
                if($value['product_type']=="Stock" ||$value['product_type']=="Order" ){
                ...
                $paymentType = input::post('payment_type');
                    ...
                    if($value['product_type']=="Install"){
                        ...
                        if($paymentType=='CC'){
                            ...
                            $sql = $this->prepare("insert into AccountDetail (file_no, sequence, date, time, account_no, debit, credit, cancelled, note)
                            values (?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, ?, ?, ?, 0, ?)"); 
                            $sql->execute([$iv_no, '1', '12-0000', (double) $final_price, 0, 'IV']);
            ...
            while( $accumStock > 0) {
               ...
               // Pro Week 7-8
               if (($CheckSoDate == '2022-07-11' && $CheckSoTime >= '12:00:00') || $CheckSoDate == '2022-07-12' || $CheckSoDate == '2022-07-13' || 
               $CheckSoDate == '2022-07-14' || $CheckSoDate == '2022-07-15' || $CheckSoDate == '2022-07-16' || 
               $CheckSoDate == '2022-07-18' || $CheckSoDate == '2022-07-19' || $CheckSoDate == '2022-07-20' || 
               $CheckSoDate == '2022-07-21' || $CheckSoDate == '2022-07-22' || $CheckSoDate == '2022-07-23') {
